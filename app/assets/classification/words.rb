...
File.open("/home/newscontext/rails_projects/articles/classification/files/"+name) do |f|
 ...
 summ=""
topicterms=["yahoo","tumblr","mayer","year","deal","user","revenu","turn","expect","york"]
	  text = text.downcase
          #text=text.gsub /nbsp/,''
	  words = text.scan(/\w+/)
	  ...
	  words.each do |key|
 	  ...
 	  words=summ.scan(/\w+/)
          words=words-topicterms
          summ=''
          words.each do |key|
              summ =summ + ' '+key
	  end       
          file=File.open("/home/newscontext/rails_projects/articles/classification/ssfiles/"+name,"w+")
            file.write(summ+"\n")
          file.close
